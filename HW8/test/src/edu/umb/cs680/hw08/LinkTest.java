...
class LinkTest {
	...
	public void verifyTargetEqualityHome() {
		...
		Directory actual = (Directory) fileSystem.getRootDirs().get(0).findByName_Link("x").getTarget();
	...
	public void verifyTargetEqualityE() {
		...
		File actual = (File) fileSystem.getRootDirs().get(0).findByName_Link("y").getTarget();
		assertArrayEquals(expected,fsElementToStringArray(actual));
	}
